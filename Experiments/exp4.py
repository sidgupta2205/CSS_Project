...
class VideoStreamer(object):
  ...
  def read_image(self, impath, img_size):
    ...
    norm_image = cv2.normalize(grayim, None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
    return norm_image
    # if grayim is None:
...
for i in range(le):
  x = np.array(li[i])
  pca = PCA(n_components=minpts)
  x=pca.fit_transform(x)
  x=x.flatten()
  imgdata.append(x)


kmeans = KMeans(n_clusters=3, random_state=42).fit(imgdata)
score = silhouette_score(imgdata, kmeans.labels_, metric='euclidean')
